...
(function () {
  ...
  if (sheet && window.ZanStore) {
    ...
    /* Hide the card option entirely when no payment page is configured. */
    const cardMethod = els.methods.find((m) => m.dataset.method === "card");
    if (cardMethod && !store.config.cardPaymentUrl) {
      cardMethod.querySelector(".method__hint").textContent = "Not set up yet";
      cardMethod.disabled = true;
    }
    ...
    if (els.cardOpen) {
      els.cardOpen.addEventListener("click", (e) => {
        if (!requireId()) { e.preventDefault(); return; }
        if (!store.config.cardPaymentUrl) {
          e.preventDefault();
          els.error.textContent = "Card payments are not switched on yet. Use UPI for now.";
        }
      });
    }
